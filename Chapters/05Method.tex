...
%
\[
u_k^\star \mid \sigma_u^2 \sim N(0, \sigma_u^2),
\qquad k = 1,\ldots,K ,
\]
%
...
In addition to the main dispersal dataset, a second dataset was used to estimate breeding values for autumn dispersal. This dataset was based on the previous dataset, but now the trait of interest is autumn dispersal. Within this dataset, the autumn dispersal phenotype was available for a subset of $1142$ nestlings from the years $2007$--$2014$ for which both nest observations and autumn observations were recorded in the same year. The remaining individuals did not have a recorded autumn dispersal phenotype and were therefore treated as missing for this trait during model fitting, but breeding values were still predicted for them.

The autumn dispersal phenotype, indicates whether an individual had dispersed from its natal island by autumn. Among the $1142$ individuals with recorded autumn dispersal status, $105$ had dispersed by autumn, and $50$ of these did not recruit the following year. This phenotype therefore captures dispersal events that occurred before recruitment, including dispersal among individuals that would not have received a dispersal phenotype in the main dataset.

\subsection{Data Subsets used in the Empirical Analysis}

% say that a reduced dataset was used for the assesment of the model performance, and that the full dataset, and the autumn dispersal data was used for the Robustness of the estimted BVs. 

Before fitting the empirical models, the dispersal dataset was examined to identify individuals whose dispersal status was not directly comparable to the main within-system dispersal phenotype. A total of $115$ individuals had dispersed into the Helgeland system from outside the study system, mostly from Vega but also from other islands. These individuals all had an observed dispersal phenotype of $y = 1$, meaning that this group provided no variation with respect to the trait. This group if individuals is therefor a biased sample from this island, and will not be representative for the whole population on this island. As a consequence of this bias, we decided to exclude these individuals when assessing model performance and ranking in the main analysis. 

%Since natal island was included as an environmental random effect and is known to influence dispersal (KILDE PÅ DET!), these individuals were excluded from both model fitting and model evaluation in the main empirical analyses of the dispersal trait.

This exclusion was applied to the analyses comparing the binomial and Gaussian BPCRR models, the comparison with the Bayesian animal model, and the ranking-based model comparisons. The excluded individuals were included again when breeding values were generated for downstream analyses. In this setting, the aim was not to evaluate model performance for the main dispersal phenotype, but to obtain breeding value predictions for the available individuals.

The autumn dispersal dataset was used only for analyses relevant to the downstream use of the estimated breeding values. Therefore, the autumn dispersal trait was included in the selection of the number of principal components and in the 10-fold cross-validation used to assess whether the estimated breeding values predicted dispersal.

\section{Simulations}

This simulation study was designed to evaluate how BPCRR performs for binary traits when the observation model is either correctly specified as binomial or approximated by a linear model. Since BPCRR represents the marker-based genetic signal through a reduced set of principal components, the simulations also allowed us to investigate how model performance depends on the number of PCs included in the fitted model. Binary phenotypes were generated under a known marker-based genetic architecture across different heritability levels, making it possible to assess both predictive performance and recovery of the underlying genetic signal.
...
u_m \sim \pi_1 \cdot 0 + (1-\pi_1) \cdot N(0,\sigma^2), \qquad m = 1, 2, \dots, M \ .
\end{equation*}
%
This induces a sparse genetic architecture, where many SNPs have no effect on the trait, while a subset of SNPs contribute to the additive genetic signal.
...
\end{equation}
%
Binary phenotypes were subsequently sampled as
\[
...
The simulated binary phenotypes were analysed using both the binomial model and the linear model. The purpose was to compare model performance when the fitted model either matched or did not match the phenotype-generating model. Since the phenotypes were generated from a Bernoulli distribution under a logit link, the binomial model represents the correctly specified observation model, whereas the linear model represents an approximation in which the binary response is analysed on the observed $0/1$ scale.

In both fitted models, the additive genetic component was represented using a marker-based principal component approach. The genetic effect was represented through principal components derived from the SNP matrix. Let $\mathbf{Z}^\star$ denote the matrix containing the principal components. The genetic component was then modeled as
%
\[
\mathbf{a} = \mathbf{Z}^\star_{K}\boldsymbol{u}^\star,
\]
%
where $\boldsymbol{u}^\star$ denotes the vector of PC-effects, and $\mathbf{Z}_K^\star$ represents the $K$ first PCs from the PC-matrix $\mathbf{Z}^\star$. In this simulation study, the fitted genetic component $\mathbf{\hat{a}}$ represents the estimated breeding values.
...
%
\[
\boldsymbol{\eta}= \beta_0 \mathbf{1}_N + \mathbf{a},
\]
%
where $\beta_0$ is the intercept and $\mathbf{a}$ is the PC-based additive genetic effects. The expected response is the probability of success,
%
\[\mathbf{p} = \mathrm{logit}^{-1}(\boldsymbol{\eta}).
\]
%
...
For the linear model, the prior calibration was performed on the observed response scale rather than on the latent liability scale. The linear model treats the binary phenotype as a continuous $0/1$ response and uses an identity link, so the PC-based genetic component is estimated on the same scale as the observed phenotype. The latent-scale additive variance $V_A$ was therefore not used directly. Instead, the target additive variance for the Gaussian prior was defined as the variance of the true simulated probabilities,
\[
V_{A,o} = \mathrm{Var}(\mathbf{p}_{\mathrm{true}}),
\]
where $\mathbf{p}_{\mathrm{true}}$ denotes the probabilities obtained by applying the inverse logit function to the simulated breeding values.
On the observed response scale, these probabilities represent the expected phenotype values implied by the simulated genetic signal. Their empirical variance was therefore used as the observed-scale analogue of the additive genetic variance.

The prior variance for the PC effects in the linear model was then calibrated by matching the expected prior variance of the PC-based genetic component to the observed-scale target variance $V_{A,o}$. Using the same variance decomposition as above, this gave
%
\[
\sigma_o^2 = \frac{V_{A,o}}{\sum_{k = 1}^K \mathrm{Var}(\mathbf{Z}^\star_k)} \ ,
\]
%
where $\sigma_o^2$ denotes the common prior variance assigned to the PC effects on the observed response scale. The corresponding fixed precision used in the INLA specification was therefore set to
\(
\tau_o = 1/\sigma_o^2.
\)
This ensured that the prior scale of the PC-based genetic component matched the scale of the fitted Gaussian response.
...
The marker effects in the phenotype-generating model were simulated from the sparse mixture distribution described above, using $\pi_1 = 0.95$ and non-zero effect variance $\sigma^2 = 10^{-4}$. For each heritability level, the simulation and model-fitting procedure was repeated $10$ times. In each replicate, an $80/20$ stratified train-test split was used, preserving the proportion of cases and controls in both sets. The same split was used for the binomial and Gaussian BPCRR models to allow paired comparison of model performance.
...
Based on the general binomial formulation in Equations~\eqref{eq:method_bin_lin_pred} and~\eqref{eq:bin_prob}, the binomial model was specified as
%
\begin{align*}
    \boldsymbol{\eta} &= \beta_0\mathbf{1}_N + \beta_{S}\mathbf{x}_{S} + \beta_{IB}\mathbf{x}_{IB}
    + d_{0,HY}\mathbf{w}_{HY} + d_{0,NI}\mathbf{w}_{NI} + \mathbf{a} \ , \\
    \mathbf{p} &= \text{logit}^{-1}(\boldsymbol{\eta}) \ ,
\end{align*}
%
where $\mathbf{x}_{S}$ is the covariate vector for sex, $\mathbf{x}_{IB}$ is the covariate vector for inbreeding, and $\mathbf{w}_{HY}$ and $\mathbf{w}_{NI}$ are design vectors for hatch year and natal island, respectively. The corresponding effects are given by $\beta_S$ and $\beta_{IB}$ for the fixed effects, and $d_{0,HY}$ and $d_{0,NI}$ for the hatch-year and natal-island random effects. The vector $\mathbf{a}$ denotes the additive genetic effects, and $\mathbf{p}$ is the vector of individual dispersal probabilities obtained by applying the inverse logit link function to the linear predictor.

The corresponding linear model was specified using the same fixed and random effects, but with a Gaussian likelihood and identity link as in Equation~\eqref{eq:method_lin_mod}:
%
\begin{equation*}
    \boldsymbol{y} = \beta_0\mathbf{1}_N + \beta_{S}\mathbf{x}_{S} + \beta_{IB}\mathbf{x}_{IB}
    + d_{0,HY}\mathbf{w}_{HY} + d_{0,NI}\mathbf{w}_{NI} + \mathbf{a} + \boldsymbol{\varepsilon} \ ,
\end{equation*}
%
where $\boldsymbol{\varepsilon}$ denotes the residual error term, and $\mathbf{y}$ is the binary dispersal response treated as a continuous response. Thus, the binomial and linear models differed in their observation model, but used the same fixed effects, random effects, and additive genetic component.
...
For the BPCRR models, the number of principal components included in the genetic component had to be selected before the final model comparisons were carried out. The value of $K$ determines the dimension of the PC-based genetic component,
\[
\mathbf{a} = \mathbf{Z}_K^\star \boldsymbol{u}^\star,
\]
and therefore controls how much genomic variation is retained in the model.

For the main dispersal trait, the binomial and Gaussian BPCRR models were fitted across the grid
\[
K \in \{100, 200, \dots, 1500\}.
\]
...
\]
%
For each value of $K$, model performance was evaluated across $10$ replicates. In each replicate, an $80/20$ stratified train-test split was generated, preserving the proportion of dispersers and non-dispersers in both sets. Predictive performance was assessed on the test set using PR-AUC. The selected number of principal components was defined as the value of $K$ that gave the highest median PR-AUC across the $10$ replicates.

\subsection{Model Comparison for the Main Dispersal Trait}
...
The aim of this comparison was to evaluate how the BPCRR models performed relative to the corresponding genomic animal models, and to assess the effect of using a binomial rather than a Gaussian observation model for the binary dispersal response. Four models were therefore compared: the binomial BPCRR model, the Gaussian BPCRR model, the binomial genomic animal model, and the Gaussian genomic animal model.
...
For each model, performance was evaluated across $10$ repeated stratified $80/20$ train-test splits. In each replicate, the model was fitted using the training set, and predictions were evaluated on the corresponding test set. The same train-test splits were used across models to allow direct comparison of predictive performance.

\subsection{Agreement Between Binomial and Gaussian BPCRR Models}

In addition to comparing predictive performance, the binomial and Gaussian BPCRR models were evaluated with respect to their agreement in ranking individuals. This comparison was performed for the main dispersal trait using the reduced dispersal dataset. Since the two models produce estimated quantities on different scales, direct comparison of absolute values was not the main focus. Instead, agreement was assessed using Spearman rank correlation.

Rank correlations were computed between estimated breeding values from the binomial and Gaussian BPCRR models. In addition, the ranking of individuals based on estimated dispersal probabilities from the binomial model was compared with the ranking based on predicted scores from the linear model. This made it possible to assess whether the two observation models identified similar individuals as having relatively high or low genetic propensity for dispersal, and whether they produced similar rankings on the observed response scale.

\subsection{Evaluation of Breeding Values for Downstream Analyses}

Since the estimated breeding values were intended for downstream analyses, their ability to predict dispersal was evaluated separately from the model-comparison analyses described above. The aim of this evaluation was not to assess the full predictive performance of each fitted observation model, but to assess whether the estimated breeding values themselves carried information about dispersal.
...
This analysis was performed for both the main dispersal trait and the autumn dispersal trait. For the main dispersal trait, the full dispersal dataset was used, including the individuals that were excluded from the model-comparison analyses. For the autumn dispersal trait, the analysis was performed using the autumn dispersal dataset, where breeding values were predicted for individuals with available genomic information and evaluated using the individuals with observed autumn dispersal status.
